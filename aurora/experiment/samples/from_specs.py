...
class SampleFromSpecs(ipw.VBox):
    ...
    def __init__(self, experiment_model, validate_callback_f,
        ...
        self.w_reset = ipw.Button(description="Reset",
                                  button_style='danger',
        ...
        self.w_select_sample_id.observe(handler=self.on_battery_id_change,
                                        names='value')
        self.w_validate.on_click(
            lambda arg: self.callback_call(validate_callback_f))
        self.w_cookit.on_click(
            lambda arg: self.callback_call(recipe_callback_f))
        ...
        return dict_to_formatted_json(
            self.experiment_model.query_available_samples(
                self.experiment_model.write_pd_query_from_dict(
                    {'battery_id': self.w_select_sample_id.value})).iloc[0])
    ...
    def _build_sample_id_options(self):
        """Returns a (option_string, battery_id) list."""
        table = self.experiment_model.query_available_samples(
            self.experiment_model.write_pd_query_from_dict(
                self.current_specs)).sort_values('battery_id')
        return [("", None)
    ...
    def display_query_result(self):
        ...
        with self.w_query_result:
            # print(f'Query:\n  {self.current_specs}')
            query_res = self.experiment_model.query_available_samples(
                self.experiment_model.write_pd_query_from_dict(
                    self.current_specs)).set_index('battery_id')[
                        self.QUERY_PRINT_COLUMNS]
            ...
            with self.w_sample_preview:
                ...
                print(
                    self.experiment_model.query_available_samples(
                        self.experiment_model.write_pd_query_from_dict(
                            {'battery_id':
                             self.w_select_sample_id.value})).iloc[0])
