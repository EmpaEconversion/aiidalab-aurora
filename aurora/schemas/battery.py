...
class BatteryComposition(BaseModel):  # TypedDict?
    ...
    def validate_composition(cls, values):
        ...
        if values['description']:
            ...
            if len(components) == 3:
                values['cathode'], values['electrolyte'], values[
                    'anode'] = components
            else:
                values['cathode'], values['electrolyte'], values['anode'] = (
                    None, None, None)
                # raise ValueError(
            ...
            values[
                'description'] = f"{values['cathode']}|{values['electrolyte']}|{values['anode']}"
        else:
            raise ValueError(
                "You must specify either a string 'description' or the components."
            )
        return values
