...
class SampleFilterWidget(ipw.VBox):
    ...
    def __init__(self, available_samples_model):
        ...
        self.w_specs_composition = ipw.Select(description="Composition:",
        ...
        self.w_specs_capacity = ipw.Select(
            ...
            layout=self.BOX_LAYOUT_1,
            style=self.BOX_STYLE_1)
        self.w_specs_form_factor = ipw.Select(description="Form factor:",
        ...
        self.w_button_reset_d = ipw.Button(description="Reset changes",
                                           ...
                                           tooltip="Reset unsaved changes",
                                           icon="refresh",
                                           style=self.BUTTON_STYLE,
                                           layout=self.BUTTON_LAYOUT)
        self.w_button_reset_f = ipw.Button(description="Reset filters",
                                           button_style='primary',
                                           tooltip="Clear filtering fields",
                                           icon="times",
                                           style=self.BUTTON_STYLE,
                                           layout=self.BUTTON_LAYOUT)
        ...
        self.w_query_result.observe(handler=self.update_extra_info,
                                    names='value')
    ...
    def _build_sample_specs_options(self, spec_field):
        ...
        query_inp = self.available_samples_model.write_pd_query_from_dict(
            sample_query_filter_dict)
        query_res = self.available_samples_model.query_available_samples(
            query_inp, project=[spec_field, 'battery_id'])
        ...
        sample_query_filter_dict = {
            'manufacturer': None,
            'composition.description': None,
            'capacity.nominal': None,
            'form_factor': None
        }
        query_inp = self.available_samples_model.write_pd_query_from_dict(
            sample_query_filter_dict)
    ...
    def _build_sample_id_options(self):
        ...
        query = self.available_samples_model.write_pd_query_from_dict(
            self.current_specs)
        table = self.available_samples_model.query_available_samples(
            query).sort_values('battery_id')
        return [("", None)
                ] + [(f"<{row['battery_id']:5}>   \"{row['metadata.name']}\"",
                      row['battery_id']) for index, row in table.iterrows()]
    ...
    def display_query_result(self):
        ...
        self.w_query_result.options = [("", None)
