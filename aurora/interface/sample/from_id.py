...
class SampleFromId(ipw.VBox):
    ...
    def __init__(self, experiment_model, validate_callback_f):
        ...
        self.w_visualize_info = ipw.Output(layout={'border': '1px solid black'})
        ...
        self.w_selected_list = ipw.Select(
            rows=10,
    ...
    def selected_sample_dict(self):
        return dict_to_formatted_json(
            self.experiment_model.query_available_samples(self.experiment_model.write_pd_query_from_dict({'battery_id': self.w_id_list.value})).iloc[0])
    ...
    def display_sample_preview(self):
        ...
        if self.w_id_list.value is not None:
            with self.w_sample_preview:
                display(self.experiment_model.query_available_samples(self.experiment_model.write_pd_query_from_dict({'battery_id': self.w_id_list.value})))
    ...
    def _build_sample_id_options(self):
        ...
        table = self.experiment_model.query_available_samples().sort_values('battery_id')
        def row_label(row):
