...
class SampleFromRecipe(ipw.VBox):
    ...
    def __init__(self, experiment_model, validate_callback_f):
        ...
        self.w_recipe_select = ipw.Select(rows=10,
                                          value=None,
                                          description="Select Recipe:",
                                          style=self.BOX_STYLE,
                                          layout=self.BOX_LAYOUT_2)
        self.w_recipe_preview = ipw.Output(layout=self.OUTPUT_LAYOUT)
        ...
        self.w_update = ipw.Button(
            description="Update",
            button_style='',
            tooltip="Update available specs and recipies",
            icon='refresh',
            style=self.BUTTON_STYLE,
            layout=self.BUTTON_LAYOUT)
        self.w_reset = ipw.Button(description="Clear",
                                  button_style='danger',
        ...
        self.w_recipe_select.observe(self.on_recipe_value_change,
                                     names='value')
        self.w_sample_metadata_name.observe(self.update_validate_button_state,
                                            names='value')
    ...
    def selected_specs_dict(self):
        return dict_to_formatted_json({
            'manufacturer':
            self.w_specs_manufacturer.value,
            'composition.description':
            self.w_specs_composition.value,
            'capacity.nominal':
            self.w_specs_capacity.value,
            'capacity.units':
            'mAh',
            'form_factor':
            self.w_specs_form_factor.value,
        })
    ...
    def update_validate_button_state(self, _=None):
        self.w_validate.disabled = (self.w_recipe_select.value is None) or (
            len(self.w_sample_metadata_name.value) == 0)
    ...
    def on_update_button_clicked(self, _=None):
        ...
        self.w_specs_manufacturer.options = self.experiment_model.query_available_specs(
            'manufacturer')
        self.w_specs_composition.options = self.experiment_model.query_available_specs(
            'composition.description')
        self.w_specs_capacity.options = self.experiment_model.query_available_specs(
            'capacity.nominal')
        self.w_specs_form_factor.options = self.experiment_model.query_available_specs(
            'form_factor')
